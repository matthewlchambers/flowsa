...
def get_region_and_division_codes():
    ...
    """
    df = pd.read_csv(input_paths.data % "Census_Regions_and_Divisions.csv",
                     dtype="str")
    return df
...
def merge_urb_cnty_pct(df):
    ...
    years_xwalk = extract_fips_years(  # from xwalk headers
        pd.read_csv(input_paths.data % 'FIPS_Crosswalk.csv', nrows=0).columns)
...
def shift_census_cnty_tbl(df, year):
    ...
    fips_xwalk = pd.read_csv(input_paths.data % 'FIPS_Crosswalk.csv', dtype=str,
